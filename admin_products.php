...
if(isset($_POST['add_product'])){
   ...
   if($price <=0 || $stock < 0){
     ...
     $message[] = 'Price and stock must be a positive value!'; 
   }else {
         $select_product_name = mysqli_query($conn, "SELECT name FROM `products` WHERE name = '$name'") or die('query1 failed');

        if(mysqli_num_rows($select_product_name) > 0){
        $message[] = 'product name already added';
         }else{
          $add_product_query = mysqli_query($conn, "INSERT INTO `products`(name, price , image , stock ) VALUES('$name', '$price', '$image' ,'$stock')") or die('query2 failed');
